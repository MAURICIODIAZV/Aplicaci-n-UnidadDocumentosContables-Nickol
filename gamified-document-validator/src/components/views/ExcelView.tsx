...
export const ExcelView: React.FC = () => {
  ...
  const handleOpenExcel = (docId: string, docName: string, fileDataUrl?: string) => {
        ...
        {excelDocuments.length === 0 ? (
          ...
          excelDocuments.map((doc) => {
            ...
            return (
              <div
                key={doc.id}
                className={`bg-white rounded-3xl p-5 border shadow-ambient hover:shadow-lift transition-all duration-300 relative overflow-hidden ${isCompleted ? 'border-emerald-200' : 'border-slate-100'
                  }`}
              >
                ...
                <div className="flex items-start justify-between">
                  ...
                  <span className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border flex items-center gap-1 ${isCompleted
                ...
                <div className="mt-4 flex items-center justify-end">
                  <button
                    onClick={() => handleOpenExcel(doc.id, doc.name, doc.fileDataUrl)}
